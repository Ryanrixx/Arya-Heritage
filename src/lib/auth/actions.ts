...
export async function createGuestSession() {
  ...
  if (existing?.value) {
    const row = await db.select().from(guests).where(eq(guests.sessionToken, existing.value)).limit(1);
    if (row.length) {
      return { ok: true, sessionToken: existing.value, id: row[0].id };
    }
  }
  ...
  return { ok: true, sessionToken, id: inserted[0].id };
}
